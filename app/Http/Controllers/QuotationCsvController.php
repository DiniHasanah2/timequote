...
$logoPath = collect([
    ...
    public_path('time_logo.png'),
        ...
        $rows[] = [ // wrap text besar; biar satu sel A..E (AfterSheet merge + wrap)
...
class QuotationCsvController extends Controller
{
    ...
    public function generateXlsx(Request $request, $versionId)
    {
        ...
        // ====== bina rows (semua nilai kewangan sebagai NUMERIC) ======
        $rows   = [];
        $rows[] = ['Quotation Report'];
        $rows[] = ['Generated Date', now()->format('d/m/Y')];

        // meta versi
        $rows[] = ['Logged Version', $catalogLabel];
        if ($catalogFrom || $catalogTo) {
            $rows[] = ['Catalog Effective', ($catalogFrom ?: '-') . ' – ' . ($catalogTo ?: '—')];
        }

        $rows[] = ['Quotation ID', $quotation->id ?? 'N/A'];
        $rows[] = ['Customer', $version->project->customer->name ?? 'N/A'];
        $rows[] = ['Project', $version->project->name ?? 'N/A'];
        $rows[] = ['Contract Duration', $contractDuration . ' Months'];

        $rows[] = []; // spacer

        // tajuk seksyen jadual
        $rows[] = ['Summary of Quotation'];
        $rows[] = ['Category', 'One Time Charges', 'KL Monthly', 'CJ Monthly', 'Total Monthly'];
        ...
        $rows[] = [
            ...
            $klCloud,
            $cjCloud,
            (float)$totalcloudSecurityCharges,
        ];
        ...
        $rows[] = [
            ...
            $klSecurity,
            $cjSecurity,
            (float)$totalSecurityCharges,
        ];

        // ===== totals (label di B, amount di C — nanti aku merge A:B dan C:E dlm AfterSheet) =====
        $rows[] = [];
        $rows[] = ['', 'ONE TIME CHARGES TOTAL', (float)$totalProfessionalCharges];
        $rows[] = ['', 'MONTHLY TOTAL',           (float)$monthlyTotal];
        $rows[] = ['', 'CONTRACT TOTAL',          (float)$contractTotal];
        ...
        return Excel::download(new QuotationArrayExport($rows, 'Quotation Summary'), $filename);
    }
...
class QuotationCsvController extends Controller
{
    ...
    public function generateXlsx(Request $request, $versionId)
    {
        ...
        // bina rows (nombor tulen; format "RM" dibuat dalam exporter)
        $rows   = [];
        $rows[] = ['Quotation Report'];
        $rows[] = ['Generated Date', now()->format('d/m/Y')];
        ...
        // Professional Services (one-time sahaja)
        $rows[] = ['Professional Services', (float)$totalProfessionalCharges, 0.0, 0.0, 0.0];

        // Managed
        $klManaged = (float) collect($managedSummary)->sum('kl_price');
        $cjManaged = (float) collect($managedSummary)->sum('cj_price');
        $rows[] = ['Managed Services', 0.0, $klManaged, $cjManaged, $klManaged + $cjManaged];

        // Network
        $rows[] = ['Network', 0.0, (float)$klTotal, (float)$cjTotal, (float)($klTotal + $cjTotal)];

        // ECS
        $rows[] = ['Compute - ECS', 0.0, (float)$klEcsTotal, (float)$cjEcsTotal, (float)($klEcsTotal + $cjEcsTotal)];

        // License
        $rows[] = ['Licenses', 0.0, (float)$licenseKL, (float)$licenseCJ, (float)($licenseKL + $licenseCJ)];

        // Storage
        $klStorage = (float) collect($storageSummary)->sum('kl_price');
        $cjStorage = (float) collect($storageSummary)->sum('cj_price');
        $rows[] = ['Storage', 0.0, $klStorage, $cjStorage, $klStorage + $cjStorage];

        // Backup
        $klBackup = (float) collect($backupSummary)->sum('kl_price');
        $cjBackup = (float) collect($backupSummary)->sum('cj_price');
        $rows[] = ['Backup', 0.0, $klBackup, $cjBackup, $klBackup + $cjBackup];

        // DR (ikut PDF – kosong kalau tiada)
        $rows[] = ['DR', 0.0, 0.0, 0.0, 0.0];

        // Cloud Security
        $klCloud = (float) collect($cloudSecuritySummary)->sum('kl_price');
        $cjCloud = (float) collect($cloudSecuritySummary)->sum('cj_price');
        $rows[] = ['Cloud Security', 0.0, $klCloud, $cjCloud, $klCloud + $cjCloud];

        // Monitoring (ikut label PDF)
        $klMonitor = (float) collect($monitoringSummary)->sum('kl_price');
        $cjMonitor = (float) collect($monitoringSummary)->sum('cj_price');
        $rows[] = ['Additional Services - Monitoring', 0.0, $klMonitor, $cjMonitor, $klMonitor + $cjMonitor];

        // Security Services
        $klSecurity = (float) collect($securitySummary)->sum('kl_price');
        $cjSecurity = (float) collect($securitySummary)->sum('cj_price');
        $rows[] = ['Security Services', 0.0, $klSecurity, $cjSecurity, $klSecurity + $cjSecurity];

        // totals
        $rows[] = [];
        $rows[] = ['', 'ONE TIME CHARGES TOTAL', (float)$totalProfessionalCharges, null, null];
        $rows[] = ['', 'MONTHLY TOTAL',           (float)$monthlyTotal,         null, null];
        $rows[] = ['', 'CONTRACT TOTAL',          (float)$contractTotal,        null, null];
        $rows[] = ['', 'SERVICE TAX (8%)',        (float)$serviceTax,           null, null];
        $rows[] = ['', 'FINAL TOTAL (Include Tax)', (float)$finalTotal,         null, null];
    ...
    public function generateCsv(Request $request, $versionId)
    {
        ...
        // Header/rows (CSV kekal string "RM" supaya senang baca tanpa styling)
        $csv->insertOne(['Quotation Report']);
        $csv->insertOne(['Generated Date', now()->format('d/m/Y')]);
        ...
        // Helper kecil untuk format RM / dash
        $rm = fn($v) => ($v > 0 ? 'RM' . number_format($v, 2) : 'RM -');
        ...
        $klManaged = collect($managedSummary)->sum('kl_price');
        $cjManaged = collect($managedSummary)->sum('cj_price');
        $csv->insertOne(['Managed Services', '', $rm($klManaged), $rm($cjManaged), $rm($klManaged + $cjManaged)]);

        // Network
        $csv->insertOne(['Network', '', $rm($klTotal), $rm($cjTotal), $rm($klTotal + $cjTotal)]);

        // ECS
        $csv->insertOne(['Compute - ECS', '', $rm($klEcsTotal), $rm($cjEcsTotal), $rm($klEcsTotal + $cjEcsTotal)]);

        // Licenses
        $csv->insertOne(['Licenses', '', $rm($licenseKL), $rm($licenseCJ), $rm($licenseKL + $licenseCJ)]);

        // Storage
        $klStorage = collect($storageSummary)->sum('kl_price');
        $cjStorage = collect($storageSummary)->sum('cj_price');
        $csv->insertOne(['Storage', '', $rm($klStorage), $rm($cjStorage), $rm($klStorage + $cjStorage)]);

        // Backup
        $klBackup = collect($backupSummary)->sum('kl_price');
        $cjBackup = collect($backupSummary)->sum('cj_price');
        $csv->insertOne(['Backup', '', $rm($klBackup), $rm($cjBackup), $rm($klBackup + $cjBackup)]);

        // DR
        $csv->insertOne(['DR', '', 'RM -', 'RM -', 'RM -']);

        // Cloud Security
        $klCloud = collect($cloudSecuritySummary)->sum('kl_price');
        $cjCloud = collect($cloudSecuritySummary)->sum('cj_price');
        $csv->insertOne(['Cloud Security', '', $rm($klCloud), $rm($cjCloud), $rm($klCloud + $cjCloud)]);

        // Monitoring
        $klMonitor = collect($monitoringSummary)->sum('kl_price');
        $cjMonitor = collect($monitoringSummary)->sum('cj_price');
        $csv->insertOne(['Additional Services - Monitoring', '', $rm($klMonitor), $rm($cjMonitor), $rm($klMonitor + $cjMonitor)]);

        // Security Services
        $klSecurity = collect($securitySummary)->sum('kl_price');
        $cjSecurity = collect($securitySummary)->sum('cj_price');
        $csv->insertOne(['Security Services', '', $rm($klSecurity), $rm($cjSecurity), $rm($klSecurity + $cjSecurity)]);
        ...
        // download (nama fail include slug versi)
        $verSlug = Str::slug($catalogLabel ?: 'legacy', '_');
        $csv->output('quotation_' . $version->id . '_' . $verSlug . '_' . now()->format('Ymd_His') . '.csv');
        die;
    }
