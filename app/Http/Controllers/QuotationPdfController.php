...
class QuotationPdfController extends Controller
{
    public function downloadTablePdf($versionId)
    {
        try {
            $version = Version::with(['project.customer', 'solution_type'])->findOrFail($versionId);
            
            // Get quotation data
            $quotation = Quotation::where('version_id', $versionId)->first();
            if (!$quotation) {
                $quotation = Quotation::create([
                    ...
                    'presale_id' => auth()->id(),
                    'status' => 'pending',
                    'quote_code' => 'Q-' . now()->format('Ymd') . '-' . strtoupper(substr(md5(uniqid()), 0, 5)),
                ]);
            }
            ...
            $filename = 'quotation_' . $version->project->project_code . '_v' . $version->version_number . '.pdf';
